...
class ModelLanguage(): # 语音模型类
	def __init__(self, modelpath):
		self.modeldir = os.path.abspath(modelpath)
		if not os.path.exists(self.modeldir):
			raise RuntimeError("modeldir %s not exists when init ModelLanguage" % self.modeldir)
		
		
	def LoadModel(self):
		self.dict_pinyin = self.GetSymbolDict('dict.txt')
		self.model1 = self.GetLanguageModel(os.path.join(self.modeldir, 'language_model1.txt'))
		self.model2 = self.GetLanguageModel(os.path.join(self.modeldir, 'language_model2.txt'))
		self.pinyin = self.GetPinyin(os.path.join(self.modeldir, 'dic_pinyin.txt'))
	...
	def GetSymbolDict(self, dictfilename):
		...
		返回读取后的字典
		'''
		txt_obj = open(dictfilename, 'r') # 打开文件并读入
	...
	def GetLanguageModel(self, modelLanFilename):
		...
		返回读取后的模型
		'''
		txt_obj = open(modelLanFilename, 'r') # 打开文件并读入
		txt_text = txt_obj.read()
		txt_obj.close()
		txt_lines = txt_text.split('\n') # 文本分割
