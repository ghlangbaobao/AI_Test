...
# 		system_type = plat.system() # 由于不同的系统的文件路径表示不一样，需要进行判断
		...
		self.datapath_stcmds = path_stcmds; # 数据存放位置根目录
	...
	def LoadDataList(self):
		...
		'''
		# 设定选取哪一项作为要使用的数据集
		filename_wavlist_thchs30 = os.path.join(self.data_list_path, 'thchs30', '%s.wav.lst' % self.type)
		filename_wavlist_stcmds = os.path.join(self.data_list_path, 'st-cmds', '%s.wav.txt' % self.type)
		filename_symbollist_thchs30 = os.path.join(self.data_list_path, 'thchs30', '%s.syllable.txt' % self.type)
		filename_symbollist_stcmds = os.path.join(self.data_list_path, 'st-cmds', '%s.syllable.txt' % self.type)
		
		# 读取数据列表，wav文件列表和其对应的符号列表
		self.dic_wavlist_thchs30,self.list_wavnum_thchs30 = get_wav_list(filename_wavlist_thchs30)
		self.dic_wavlist_stcmds,self.list_wavnum_stcmds = get_wav_list(filename_wavlist_stcmds)
		
		self.dic_symbollist_thchs30,self.list_symbolnum_thchs30 = get_wav_symbol(filename_symbollist_thchs30)
		self.dic_symbollist_stcmds,self.list_symbolnum_stcmds = get_wav_symbol(filename_symbollist_stcmds)
	...
	def GetSymbolList(self):
		...
		返回一个列表list类型变量
		'''
		txt_obj=open('dict.txt','r') # 打开文件并读入
